    ...
    if(isset($_POST['upload'])){
        if($_FILES['ucas_file']['name']){
            ...
            while($row = fgetcsv($file)){
                $value = "'".implode("','",$row)."'";
                $stmt = $pdo->prepare("INSERT INTO UCAS(UCAS_id, first_name, sur_name, course_name, email, gpa, gender, date_of_birth, address, contact) VALUES(".$value.")");
                $res=$stmt->execute();
            }
            if($res){
                echo "imported";
...
<table border="1">
    ...
    <?php  
        $stmt = $pdo->prepare("SELECT * FROM UCAS WHERE case_status=0");
        $stmt->execute();
        foreach ($stmt as $data){?>
            <tr>
                <td><a href="index.php?ucId=<?php echo $data['UCAS_id']; ?>">Create Case</a></td>
